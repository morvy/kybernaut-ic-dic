...
class OrderCommentTest extends TestCase {
    ...
    public function testViesValidationScenarios(\$vat_number, \$vies_validator_response, \$expected_vies_result_text, \$expected_vies_details_text) {
        ...
        if (\$vies_validator_response instanceof \$vies_exception_fqcn) {
            ...
            // \$logger_instance_mock = WP_Mock::mock('KybernautIcDic\Logger'); // Already in setUp
            // \$logger_instance_mock->shouldReceive('log')->once(); // Or more specific matching message
            // WP_Mock::userFunction('KybernautIcDic\Logger::getInstance', ['return' => \$logger_instance_mock]);
        }
